...
def get_frequencies(chunk_bits, data_to_send):
    chunk = []
...
def extract_packet(freqs):
    ...
    print( bytearray(decode_bitchunks(BITS, bit_chunks)))
            ...
            try:
                ...
                if '201704148' in byte_stream:
                    ...
                    for freq in frequencies:
                        sample = (np.sin(2 * np.pi * np.arange(SAMPLE_RATE * DURATION) * freq / SAMPLE_RATE)).astype(np.float32).tobytes()
                        '''
